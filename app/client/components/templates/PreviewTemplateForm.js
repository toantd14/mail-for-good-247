...
const PreviewTemplateForm = (props) => {
  ...
  return (
    <div>
      <h3>
        <i className="fa fa-list text-green" aria-hidden="true" /> - {form.templateName || "Not set"}
      </h3>

      <hr />

      <h3>
        <strong>From: {`${form.fromName || "Not set"} <${form.fromEmail || "Not set"}>`}</strong>
      </h3>
      <h4>
        <strong>Subject: {`${form.emailSubject || "Not set"}`}</strong>
      </h4>

...
export class CampaignViewComponent extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.campaigns && nextProps.campaigns.length && !this.props.campaigns.length) {
      // Guarded and statement that confirms campaigns is in the new props, confirms the array isn't empty, and then confirms that current props do not exist
  ...
  render() {
    const { thisCampaign, haveShownMessage, showTestSendModal, testEmail } = this.state;

    const { sendCampaignResponse, sendCampaignStatus, isPostingSendCampaign, isPostingSendTest, isGetting } =
      this.props;

    const downloadUnsentSubscribersUrl = encodeURI(
      `${window.location.origin}/api/campaign/subscribers/csv?campaignId=${thisCampaign.id}&sent=false`
    );
            ...
            <div className="box-body">
              {sendCampaignResponse && haveShownMessage && (
                <p className={sendCampaignStatus === 200 ? "text-green" : "text-red"}>
                  <i className={sendCampaignStatus === 200 ? "fa fa-check" : "fa fa-exclamation"} />{" "}
                  {sendCampaignResponse.split(".")[0]}.<br /> <br /> {sendCampaignResponse.split(".")[1]}.
                </p>
              )}
              ...
              <div className="form-inline">
                <button
                  disabled={!["ready", "interrupted"].includes(status)}
                ...
                <button
                  ...
                  type="button"
                  onClick={this.openTestSendModal}
                >
                  Send a test email
                </button>
                <button
                  ...
                  style={{ marginLeft: "1rem" }}
                  onClick={() => {
                    window.location = downloadUnsentSubscribersUrl;
                  }}
                >
                  Export unsent
                </button>
                <button
                  ...
                  style={{ marginLeft: "1rem" }}
                  type="button"
                  onClick={this.stopSending.bind(this)}
                >
                  Stop sending
                </button>
              </div>
              ...
              <Modal show={showTestSendModal} onHide={this.closeTestSendModal}>
                ...
                <Modal.Footer>
                  ...
                  <Button bsStyle="primary" onClick={this.sendTestEmail}>
                    Send Test Email
                  </Button>
                </Modal.Footer>
              </Modal>
              ...
              <Modal show={this.state.showSendModal} onHide={this.closeSendModal}>
                <Modal.Header closeButton>
                  <Modal.Title>
                    Are you ready to send this campaign to{" "}
                    {thisCampaign.totalCampaignSubscribers
                      ? thisCampaign.totalCampaignSubscribers.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")
                      : 0}{" "}
                    subscribers?
                  </Modal.Title>
                </Modal.Header>
    ...
    return <div>{canRender && renderCampaignView()}</div>;
  }
}

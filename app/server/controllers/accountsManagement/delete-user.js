...
module.exports = function (req, res) {
  Promise.all([db.user.getIsAdmin(req.body.queriersEmail), db.user.checkIfUserExists(req.body.email)])
    .then((checks) => {
      if (checks[0] === false) {
        ...
        db.user
          ...
          .catch((error, errorStatus) => {
            res.status(500).send("The application has'nt been able to delete the user.");
          })
          .then((success) => {

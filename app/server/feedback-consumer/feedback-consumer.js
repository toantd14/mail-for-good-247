  ...
  Settings.findAll({})
    .then((settings) => {
      ...
      settings.forEach((setting) => {
        debug(
          "Initialising a consumer using AWS access key: %s with the queue URL:  %s",
          setting.amazonSimpleEmailServiceAccessKey,
          setting.amazonSimpleQueueServiceUrl
        );
        try {
          consumers.push(
            createConsumer(
              setting.region,
              setting.amazonSimpleEmailServiceAccessKey,
              setting.amazonSimpleEmailServiceSecretKey,
              setting.amazonSimpleQueueServiceUrl
            )
          );
        } catch (e) {

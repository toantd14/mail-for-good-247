...
module.exports = async function (arrayOfIds, campaignInfo, whiteLabelUrl) {
  ...
  for (let i = 0; i < arrayOfIds.length; i++) {
    ...
    if (arrayCampaignInfo[i].unsubscribeLinkEnabled) {
      arrayCampaignInfo[i].emailBody = insertUnsubscribeLink(
        arrayCampaignInfo[i].emailBody,
        subscribers[i].unsubscribeKey,
        arrayCampaignInfo[i].type,
        whiteLabelUrl
      );
    }
    ...
    if (arrayCampaignInfo[i].trackLinksEnabled) {
      arrayCampaignInfo[i].emailBody = wrapLink(
        arrayCampaignInfo[i].emailBody,
        links[i].trackingId,
        arrayCampaignInfo[i].type,
        whiteLabelUrl
    ...
    if (arrayCampaignInfo[i].trackingPixelEnabled) {
      arrayCampaignInfo[i].emailBody = insertTrackingPixel(
        arrayCampaignInfo[i].emailBody,
        opens[i].trackingId,
        arrayCampaignInfo[i].type,
        whiteLabelUrl

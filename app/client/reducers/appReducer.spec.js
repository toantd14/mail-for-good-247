...
import { requestProfile, completeProfile, receiveNotification, consumeNotification } from "../actions/appActions";
...
describe("(Reducer/Action Creator) app", () => {
  ...
  it("should handle REQUEST_WS_PROFILE", () => {
    expect(profile(undefined, requestProfile())).to.deep.equal({
  ...
  it("should handle COMPLETE_WS_PROFILE", () => {
    ...
    expect(profile(undefined, completeProfile(mockData))).to.deep.equal({
      ...initialState.profile,
      user: { ...mockData },
    });
  });

...
const PreviewCampaignForm = (props) => {
  ...
  return (
    <div>
      {form.listName && (
        <h3>
          <i className="fa fa-list text-green" aria-hidden="true" /> - {form.listName}
        </h3>
      )}
      <h3>
        <i className="fa fa-flag text-green" aria-hidden="true" /> - {form.campaignName || form.name}
      </h3>

      <hr />

      <h4>
        <strong>From: {`${form.fromName} <${form.fromEmail}>`}</strong>
      </h4>
      <h4>
        <strong>Subject: {`${form.emailSubject}`}</strong>
      </h4>
      {type === "HTML" ? (
        <blockquote>
        ...
        <textarea className="form-control" disabled style={{ width: "100%", minHeight: "60vh" }} value={text} />
      )}

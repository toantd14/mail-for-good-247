...
module.exports = (req, res, io, redis) => {
  ...
  function* sendCampaign() {
    ...
    const { totalListSubscribers, totalEmailsToSend } = yield countListSubscribers(
      campaignInfo.listId,
      quotas.AvailableToday,
      campaignId
    );
    ...
    res.send(
      howLongEmailingWillTake(totalEmailsToSend, quotas.AvailableToday, quotas.MaxSendRate, campaignInfo.status)
    );
    ...
    ses.getSendQuota((err, data) => {
      if (err) {
        ...
        if (MaxSendRate <= 1) {
          res
            .status(400)
            .send({ message: "You are currently in Sandbox Mode. Please contact Amazon to get this lifted." });
          return;
        }
      ...
      .then((total) => {
        ...
        if (totalEmailsToSend > AvailableToday && process.env.NODE_ENV === "production") {
          res
            .status(400)
            .send({
              message: `This list exceeds your 24 hour allowance of ${AvailableToday} emails. Please upgrade your SES limit.`,
            });
        } else {
...
function howLongEmailingWillTake(totalEmailsToSend, AvailableToday, MaxSendRate, status) {
  ...
  if (status == "ready") {
    formattedMessage = `Your campaign is being sent to ${totalEmailsToSend.toLocaleString(
      "en-GB"
    )} subscribers, it should be done `;
  } else if (status == "interrupted") {
    formattedMessage = `Campaign sending resumed - sending to remaining ${totalEmailsToSend.toLocaleString(
      "en-GB"
    )} subscribers, it should be done `;
  }
  ...
  formattedMessage += ` Your Amazon limit for today is now ${emailsLeftAfterSend
    .toString()
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",")} emails.`;

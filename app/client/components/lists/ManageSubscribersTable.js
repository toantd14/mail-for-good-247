...
export default class ManageSubscribersTable extends Component {
  ...
  onExportToCSV() {
    let downloadUrl = `${window.location.origin}/api/list/subscribers/csv?listId=${
      this.props.listId
  ...
  customExportCSVButton() {
    return <ExportCSVButton btnText="Export all to CSV" onClick={this.onExportToCSV.bind(this)}></ExportCSVButton>;
  }
  ...
  render() {
    ...
    let additionalColumns = this.state.additionalFields.map((field) => {
      ...
      return (
        <TableHeaderColumn
          dataField={"additionalData." + field}
          key={"additionalData." + field}
          dataFormat={this.formatAdditionalData}
          formatExtraData={"additionalData." + field}
        >
          {field}
        </TableHeaderColumn>
      );
    ...
    let columns = [
      <TableHeaderColumn dataField="id" key="id" hidden={true}>
        id
      </TableHeaderColumn>,
      <TableHeaderColumn dataField="email" key="email">
        Email
      </TableHeaderColumn>,
    ]
      .concat(additionalColumns)
      .concat([
        <TableHeaderColumn dataField="createdAt" key="createdAt" dataFormat={this.formatDate} width="150">
          Created
        </TableHeaderColumn>,
        <TableHeaderColumn dataField="updatedAt" key="updatedAt" dataFormat={this.formatDate} width="150">
          Updated
        </TableHeaderColumn>,
        <TableHeaderColumn

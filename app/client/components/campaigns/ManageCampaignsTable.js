...
const ManageCampaignsTable = ({ data, deleteRows, getCampaignView }) => {
  ...
  const options = {
    ...
    afterDeleteRow: (rows) => {
      // Optimistic update, can assume request will succeed. 'Rows' has format [...rowKey] where rowKey is a list primary key
      deleteRows(rows);
    },
    handleConfirmDeleteRow: (next) => {
      next();
    }, // By default, react-bootstrap-table confirms choice using an alert. We want to override that behaviour.
  };
  ...
  const countBounced = (data) => {
    return (
      data["campaignanalytic.permanentBounceCount"] +
      data["campaignanalytic.transientBounceCount"] +
      data["campaignanalytic.undeterminedBounceCount"]
    );
  };
  ...
  const actionButtonsFormatter = (cell, row) => {
    return (
      <a href="#" onClick={getCampaignView.bind(this, row)}>
        Manage
      </a>
    );
  };
  ...
  return (
    ...
    >
      ...
      <TableHeaderColumn dataField="slug" hidden={true}>
        Slug
      </TableHeaderColumn>
      <TableHeaderColumn dataField="name" dataAlign="center" dataSort={true} width="450">
        Name
      </TableHeaderColumn>
      <TableHeaderColumn dataAlign="center" width="150" dataFormat={actionButtonsFormatter.bind(this)}>
        Actions
      </TableHeaderColumn>
      <TableHeaderColumn dataField="status" dataAlign="center" dataSort={true} dataFormat={statusFormatter} width="150">
        Status
      </TableHeaderColumn>
      <TableHeaderColumn
        dataField="campaignanalytic.totalSentCount"
        dataAlign="center"
        dataSort={true}
        csvHeader="sent"
      >
      ...
      <TableHeaderColumn
        ...
        dataAlign="center"
        dataSort={true}
        dataFormat={deliveredFormatter}
        csvFormat={deliveredFormatter}
      >
        Delivered
      </TableHeaderColumn>
      <TableHeaderColumn
        ...
        dataAlign="center"
        dataSort={true}
        dataFormat={bouncedFormatter}
        csvFormat={bouncedFormatter}
      >
        Bounced
      </TableHeaderColumn>
      <TableHeaderColumn
        dataField="campaignanalytic.complaintCount"
        dataAlign="center"
        dataSort={true}
        csvHeader="complaints"
      >
        Complaints
      </TableHeaderColumn>
      <TableHeaderColumn
        dataField="campaignanalytic.clickthroughCount"
        dataAlign="center"
        dataSort={true}
        dataFormat={clickthroughsFormatter}
        csvHeader="clickthroughs"
      >
        Clickthroughs
      </TableHeaderColumn>
      <TableHeaderColumn
        dataField="campaignanalytic.openCount"
        dataAlign="center"
        dataSort={true}
        dataFormat={opensFormatter}
        csvHeader="opens"
      >
        Opens
      </TableHeaderColumn>

...
const CreateTemplateForm = (props) => {
  const { touch, valid, pristine, submitting, nextPage, reset, validationFailed, textEditorType, passResetToState } =
    props;
  ...
  return (
    <form onSubmit={resetFormAndSubmit}>
      ...
      <div>
        <label>
          <Field
            disabled={textEditorType == "Plaintext"}
      ...
      <div>
        <label>
          <Field disabled={textEditorType == "Plaintext"} name="trackLinksEnabled" component="input" type="checkbox" />{" "}
          Track link clickthroughs, syntax: {`{linklabel/http://mylinktotrack.com}`}. Available for HTML emails only.{" "}
        </label>
      </div>
      <div>
        <label>
          <Field name="unsubscribeLinkEnabled" component="input" type="checkbox" /> Add unsubscribe link
        </label>
      </div>
      <hr />
      ...
      <h3>
        ...
        <a
          id="fb-icon"
          title="List of Emoticons for facebook"
          target="_blank"
          href="http://www.symbols-n-emoticons.com/p/facebook-emoticons-list.html"
        >
          FB-Icon
        </a>
        ]
      ...
      <div className="box-footer">
        <div className="btn-group">
          <button className="btn btn-success btn-lg btn-hug" type="submit" disabled={pristine || submitting}>
            Next Step
          </button>
          <button

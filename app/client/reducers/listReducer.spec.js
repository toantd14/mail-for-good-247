...
import { manageListSubscribers, createList, manageList } from "./listReducer";
...
describe("(Reducer/Action Creator) list", () => {
  ...
  it("should handle REQUEST_GET_LIST_SUBSCRIBERS", () => {
    ...
    expect(manageListSubscribers(undefined, requestGetListSubscribers(mockListId))).to.deep.equal({
  ...
  it("should handle COMPLETE_GET_LIST_SUBSCRIBERS", () => {
    ...
    expect(
      manageListSubscribers(
        undefined,
        completeGetListSubscribers(mockSubscribers, mockTotalListSubscribers, mockAdditionalFields)
      )
    ).to.deep.equal({
      ...initialState.manageListSubscribers,
      isGetting: false,
  ...
  it("should handle COMPLETE_DELETE_LIST_SUBSCRIBERS", () => {
    ...
    expect(manageListSubscribers(undefined, completeDeleteListSubscribers(mockSubscribers))).to.deep.equal({
      ...initialState.manageListSubscribers,
      subscribers: mockSubscribers,
    });
  });
  ...
  it("should handle REQUEST_ADD_SUBSCRIBERS", () => {
    ...
    expect(createList(undefined, requestAddSubscribers(mockUpload))).to.deep.equal({
  ...
  it("should handle COMPLETE_ADD_SUBSCRIBERS", () => {
    expect(createList(undefined, completeAddSubscribers())).to.deep.equal({
      ...initialState.createList,
      isPosting: false,
      upload: null,
    });
  ...
  it("should handle REQUEST_GET_LISTS", () => {
    expect(manageList(undefined, requestGetList())).to.deep.equal({
  ...
  it("should handle COMPLETE_GET_LISTS", () => {
    ...
    expect(manageList(undefined, completeGetList(mockLists))).to.deep.equal({
      ...initialState.manageList,
      lists: mockLists,
      isGetting: false,
    });
  ...
  it("should handle COMPLETE_DELETE_LISTS", () => {
    const mockLists = "something";
    expect(manageList(undefined, completeDeleteLists(mockLists))).to.deep.equal({
      ...initialState.manageList,
      lists: mockLists,
    });
  });

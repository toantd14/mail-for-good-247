  ...
  Promise.all([validateListBelongsToUser, validateCsvDoesNotContainErrors])
    .then((values) => {
      ...
      function sendFinalNotification() {
        ...
        const notification = {
          message: `"${filenameWithoutExtension}" successfully uploaded (${numberProcessed
            .toString()
            .replace(/\B(?=(\d{3})+(?!\d))/g, ",")} rows)`,
          icon: "fa-list-alt",
          iconColour: "text-green",
      ...
      function updateListStatusReady() {
        db.listsubscriber
          ...
          .then((total) => {
            db.list
              .update({ status: "ready", total }, { where: { id: listId } })
              .then(() => {

...
module.exports = (credentials, callback) => {
  ...
  async.waterfall(
    [
      async.apply(createSnsTopics, {
        ses: { email: credentials.email },
        sqs: { url: "", arn: "" },
        sns: { bounce: { arn: "" }, complaint: { arn: "" } },
      }),
      createSqsQueue,
      subscribeSnsToSqs,
      subscribeSesToSns,
    ...
    (err, result) => {
      if (err) {
        ...
        if (!identities.includes(config.ses.email)) {
          callback(
            new Error(
              "SES email address was not found - have you created and verified this email in the SES settings page in the AWS console?"
            )
          );
        } else {
          async.parallel(

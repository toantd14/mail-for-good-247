...
export class ImportCSVComponent extends Component {
  ...
  handleErrorWithFile(err) {
    // Err is a string informing user of an issue with the csv
    this.props.notification({
      // Ref https://github.com/pburtchaell/react-notification & https://github.com/pburtchaell/react-notification/blob/master/src/notification.js
      message: err,
    });
  }
  ...
  handleNewFile(file) {
    const callback = (results) => {
      if (!results.meta.fields.some((field) => field.toLowerCase() === "email" || "e-mail")) {
        // Check if any header field is labeled email
        this.handleErrorWithFile(
          'Please ensure the CSV file contains at least one column field labeled "email/e-mail" (check the first row)'
        );
      } else {
        ...
        const errorIsNotSignificant = (errors) => {
          // Check that thrown errors are actually errors
          if (errors.length) {
            return errors.every((i) => {
              // Put allowed errors below
            ...
            <Col md={12}>
              <div className="box-header">
                ...
                <div className="progress">
                  <div
                    style={{ width: `${this.props.upload}%` }}
                    className="progress-bar progress-bar-aqua"
                    role="progressbar"
                    aria-valuenow={this.props.upload}
                    aria-valuemin="0"
                    aria-valuemax="100"
                  >
            ...
            <Col md={12}>
              <div className="box-header">
                ...
                <h3 className="box-title">
                  Please ensure the first row of the CSV file contains column names. This row must contain an email
                  header.
                </h3>
              </div>
              ...
              and clicking the label is equivalent to clicking the input*/}
                {!this.state.subscribers && !this.state.errors && (
                  <label htmlFor="fileInput" className="btn pull-left btn-lg btn-primary">
                    Import
                  </label>
                )}
                <FormControl
                ...
                <div className="btn-group">
                  {this.state.subscribers && (
                    <button
                      ...
                      className="btn pull-left btn-lg btn-success"
                      type="submit"
                  ...
                  {(this.state.subscribers || this.state.errors) && (
                    <button
                      style={{ margin: "1em", width: "160px" }}
                      className="btn pull-right btn-lg btn-danger"

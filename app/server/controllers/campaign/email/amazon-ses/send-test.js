  ...
  Promise.all([campaignBelongsToUser, getAmazonKeysAndRegion])
    .then(() => {
      ...
      if (campaignId) {
        ...
        campaign.emailBody = campaign.emailBody.replace(/{{.*}}/gm, (match) => {
        ...
        if (campaign.unsubscribeLinkEnabled) {
          campaign.emailBody = insertUnsubscribeLink(
            campaign.emailBody,
            "example-unsubscribe-id",
            campaign.type,
            whiteLabelUrl
          );
        }
      ...
      if (templateId) {
        //replace all variables by their names to not get their '{}' processed by wrapLink()
        template.emailBody = template.emailBody.replace(/{{.*}}/gm, (match) => {
          return match.slice(2, -2);
        });
        // Modify email body for analytics
        ...
        if (template.unsubscribeLinkEnabled) {
          template.emailBody = insertUnsubscribeLink(
            template.emailBody,
            "example-unsubscribe-id",
            template.type,
            whiteLabelUrl

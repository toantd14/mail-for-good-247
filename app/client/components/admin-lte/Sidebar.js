      ...
      <section className="sidebar">
        <div className="user-panel">
          <div className="pull-left image">
            <img
              ...
              src={user.picture || "https://placeholdit.imgix.net/~text?txtsize=15&txt=160%C3%97160&w=160&h=160"}
              alt="User Image"
            />
          </div>
        ...
        <ul className="sidebar-menu">
          ...
          {!anotherAccountIsActive ? (
            <SidebarLink to="/" icon="fa-tachometer">
              Dashboard
            </SidebarLink>
          ) : (
          ...
          {!anotherAccountIsActive || (activeAccount.lists && activeAccount.lists !== "none") ? (
            <SidebarTreeview name="Lists" icon="fa-list">
              <SidebarLink to="/lists/create">Create List</SidebarLink>{" "}
              {/* Import from CSV etc. Keep this isolated to importing */}
              <SidebarLink to="/lists/manage">Manage Lists</SidebarLink>{" "}
              {/* RUD, Export list, statistics, add subscriber, duplicate list, combine list */}
            </SidebarTreeview>
          ) : (
          ...
          {!anotherAccountIsActive || (activeAccount.templates && activeAccount.templates !== "none") ? (
            <SidebarTreeview name="Templates" icon="fa-file-text">
              <SidebarLink to="/templates/create">Create Template</SidebarLink>{" "}
              {/* typeofcampaign (html, plain etc), use template, steps= 1. to who - 2. campaign info (name, from, subject, from email, options for tracking) 3. Template 4. Write the actual email 5. send & confirm*/}
              <SidebarLink to="/templates/manage">Manage Templates</SidebarLink>{" "}
              {/* delete, resend, edit, view report (analytics) */}
            </SidebarTreeview>
          ) : (
          ...
          {!anotherAccountIsActive || (activeAccount.campaigns && activeAccount.campaigns !== "none") ? (
            <SidebarTreeview name="Campaigns" icon="fa-envelope">
              <SidebarLink to="/campaigns/create">Create Campaign</SidebarLink>{" "}
              {/* typeofcampaign (html, plain etc), use template, steps= 1. to who - 2. campaign info (name, from, subject, from email, options for tracking) 3. Template 4. Write the actual email 5. send & confirm*/}
              <SidebarLink to="/campaigns/manage">Manage Campaigns</SidebarLink>{" "}
              {/* delete, resend, edit, view report (analytics) */}
            </SidebarTreeview>
          ) : (
          ...
          {!anotherAccountIsActive ? (
            <SidebarLink to="/settings" icon="fa-cog">
              Settings
            </SidebarLink>
          ) : (
            <DisabledLink icon="fa-cog">Settings</DisabledLink>
          )}{" "}
          {/* User's settings e.g. AWS keys */}
        </ul>
      </section>
    </aside>
  );
